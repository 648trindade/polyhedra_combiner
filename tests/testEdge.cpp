...
TEST_CASE("Edge edges_intersect line")
{
    ...
    for (float x : {-1.f, 1.f})
    {
        ...
        auto [intersect, point] = a.intersect(line_point, line_vector);
        REQUIRE(intersect);
    ...
    for (float x : { -1.f, 1.f })
    {
        for (bool y : { true, false })
        {
            ...
            auto [intersect, point] = a.intersect(line_point, line_vector.normalize());
            REQUIRE(intersect);
...
TEST_CASE("Edge edges_intersect edge")
{
    const Edge a { Vertex { 0, 0, 0 }, Vertex { 0, 3, 0 } };
